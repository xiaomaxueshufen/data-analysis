...
- `ab_test`：检查随机分流、效应、显著性、异质性与护栏；
- `causal`：估计动作带来的增量，缺少识别条件时降级为关联诊断；
- `segmentation`：建立不泄漏结果的用户分层并连接到动作；
...
```json
{
  ...
  "primary_method": "anomaly|funnel|metric_system|ab_test|causal|segmentation|contribution|ratio_decomp",
  "secondary_methods": [],
