...
- L0：一个可定位的计算结果即可；数字必须能从**结论自己引用的证据条目**中按量纲与方向匹配；
...
数字溯源、对账联动、方向与符号、量纲匹配、`--strict` / `--lenient-numbers` 等开关的完整规则见 `references/ops-catalog.md` 的「发布前真实性检查」一节——那里是唯一出处，本文件不重复。
...
7. L2 及以上因果措辞是否挂上 `identification_strategy`，且在白名单内（**漏填同样失败**）；
8. 探索性结论数是否超过阈值，超过时是否在 limitations 中标注多重比较风险；
9. 表内存在合计/小计行时，是否跑过 `da_reconcile.py`，且结论没有引用对账不通过的数字；
10. 每个关键数字的 `evidence_ids` 是否指得到实际证据条目（不是只写了占位 id）。
